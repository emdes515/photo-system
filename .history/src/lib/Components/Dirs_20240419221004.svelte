<script>
	...
	let tablicaObiektow = Object.keys(json.directories).map((key) => {
		if (json.directories[key] === null) {
			json.directories[key] = {} // domyślna wartość
		}
		return { name: key, ...json.directories[key] }
	})
